// Per-user persistence of the task-number pad width + visibility. The number
// PREFIX used to be a single free-text value here too, but that made every task
// (of every team) show the same shared prefix — it is now derived automatically
// per row from the task's owning team (see domain/team-code.ts), so there is no
// prefix preference to persist any more. Pad width / visibility remain personal
// VIEW preferences — they change only the on-screen label, not any stored data —
// so they still live in localStorage keyed by event: no server round-trip, $0.
