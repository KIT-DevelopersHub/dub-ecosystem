...
describe("dev-seed (real LMB conference data)", () => {
  ...
  it("renders the 41 work-packages + 128 WBS leaves as a two-level tree, each with a bar", async () => {
    ...
    // A row WITH children (a work-package) carries null own dates — its span is derived
    // from children (mirrors gantt-service dto.toRow, which nulls parent dates). Every row
    // WITHOUT children (leaves + childless top-level work-packages like 3.3) carries real
    // dates. The client rolls the parent span up so every row still shows a bar.
    ...
    const rolled = rollupRowDates(dto.rows);
    expect(rolled.every((r) => r.startsAt !== null && r.endsAt !== null)).toBe(true);
