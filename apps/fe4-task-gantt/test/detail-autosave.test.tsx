...
// burst of edits into ONE patch, the 保存中…→保存しました / 保存に失敗しました
// (rollback) indicator states, and that free-text 詳細 (description) auto-saves too.
...
describe("TaskDetailPanel — auto-save (保存ボタン廃止)", () => {
  ...
  it("auto-saves a free-text 詳細 (description) edit", async () => {
    const onSave = vi.fn();
    render(<TaskDetailPanel {...base({ onSave, autosaveDebounceMs: 0 })} />);
    fireEvent.change(screen.getByTestId("fe4-detail-description"), { target: { value: "背景メモ" } });
    await waitFor(() => expect(onSave).toHaveBeenCalled());
    const [patch] = onSave.mock.calls[0]!;
    expect(patch.description).toBe("背景メモ");
  });
