...
describe("③ マイタスク発行モーダル: 先行/親タスクを設定できる", () => {
  ...
  it("トップレベル新規タスクは先行タスクを設定でき、submit の draft.dependsOnIds に反映される", async () => {
    const { onCreate } = renderModal();
    fireEvent.change(screen.getByTestId("fe4-mytask-create-event"), { target: { value: EVT } });
    // 親なし（トップレベル）→ 先行候補はトップレベルの t1/t2。t2 を先行に追加。
    fireEvent.focus(screen.getByTestId("fe4-mytask-create-deps-input"));
