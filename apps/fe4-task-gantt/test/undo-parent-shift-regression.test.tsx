...
describe("Parent bar drag undo is 1 step per drag (subtree shift)", () => {
  it("each Ctrl-Z reverses exactly one parent move — never overshoots", async () => {
    ...
    // one undo -> coord 1 for the child (the parent's own dates are null in the read model
    // — a work-package span is derived from its children — so the child coordinate is the
    // source of truth; the parent bar visually tracks it via rollup).
