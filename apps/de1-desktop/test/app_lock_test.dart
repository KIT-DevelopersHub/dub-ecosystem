...
// successful biometric prompt, or stay locked on a failed/cancelled one (with
// an explicit fallback to manual login so it's never a dead end).
...
void main() {
  ...
  test('fallbackToManualLogin unlocks without touching saved credentials',
      () async {
    final container = _containerFor(
      supported: true,
      hasCreds: true,
    ...
    container.read(appLockControllerProvider.notifier).fallbackToManualLogin();
