...
/// user's Dub credentials, so a biometric unlock (Touch ID on macOS, Face ID /
/// Touch ID on iOS, Windows Hello on Windows) can auto-fill the web login form
/// on the next launch.
///
/// Backed by the OS secure store — macOS/iOS Keychain, Windows Credential
/// Manager (DPAPI). The password is **never** written to a plain file or
/// logged. On macOS we opt out of the data-protection keychain so the store
/// works in an unsigned/dev build without a provisioning-profile
/// keychain-access-group; on iOS the standard Keychain (gated by
/// `first_unlock`) is used since the app is provisioned/signed.
