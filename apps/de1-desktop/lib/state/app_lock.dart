...
/// macOS, Face ID / Touch ID on iOS, Windows Hello on Windows, with a
/// passcode/password fallback everywhere).
///
/// This is the *entire* native purpose of the mobile/desktop shell: everything
/// else is the real Web SPA in a WebView. Passing this gate is what authorises
/// the shell to auto-fill the saved Dub credentials into the web login form.
///
...
class BiometricGate {
  ...
  /// Prompts the OS authentication UI (Face ID / Touch ID / Windows Hello /
  /// device passcode, depending on the platform and what's enrolled). Returns
...
class AppLockController extends StateNotifier<LockState> {
  ...
  /// Falls back to a plain manual login (never a dead end): skips the
  /// biometric gate for this launch without touching the saved credentials, so
  /// the WebView opens and the user can sign in by hand as usual.
  void fallbackToManualLogin() {
