...
/// Owns the *consent* around credential autofill. Capturing a typed login does
/// not silently persist it; instead the credentials are held in memory
/// ([_pending]) until the user explicitly opts in (via the post-login "enable
/// Face ID / Touch ID?" dialog or the settings toggle). Only then are they
/// written to the OS secure store, which is what arms the biometric launch
/// gate + web-form auto-fill on the next launch.
class AutofillController extends StateNotifier<AutofillState> {
  ...
  /// Whether the post-login "enable Face ID / Touch ID?" dialog should be
  /// shown: autofill isn't on yet, we have a freshly typed login to offer to
  /// save, and the user hasn't already declined this session.
