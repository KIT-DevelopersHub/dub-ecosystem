...
/// A single-axis scroll delta: whichever of [dx]/[dy] was dominant for one
/// wheel/trackpad update, with the other axis forced to zero.
class ScrollAxisDelta {
  const ScrollAxisDelta(this.dx, this.dy);
  final double dx;
  final double dy;
}

/// Picks a single scroll axis for one mouse-wheel / trackpad-pan update.
///
/// [Dub vendored patch — see apps/de1-desktop/third_party/README.md]
/// Sending a non-zero horizontal delta to WebView2's
/// `ICoreWebView2CompositionController.SendMouseInput` alongside (or
/// interleaved with) a vertical delta breaks vertical scrolling for the rest
/// of that scroll session — a confirmed upstream bug, reported and root-
/// caused against this exact code path:
/// https://github.com/pichillilorenzo/flutter_inappwebview/issues/2503
/// https://github.com/pichillilorenzo/flutter_inappwebview/issues/2511
///
/// Two-finger trackpad scrolling on Windows almost always carries a small
/// amount of horizontal jitter even when the user's intent is purely
/// vertical, so forwarding both axes unconditionally (the upstream 0.6.0
/// behavior below) makes vertical scroll fail intermittently — which is
/// exactly the "many places vertical scroll just doesn't work" symptom this
/// patch fixes. Only the axis with the larger magnitude for a given update is
/// forwarded; the other is zeroed instead of dropped so a real gesture that
/// stays on one axis (a normal vertical scroll, or a deliberate horizontal
/// swipe) still works.
ScrollAxisDelta dominantAxisScrollDelta(double dx, double dy) {
  if (dx.abs() > dy.abs()) {
    return ScrollAxisDelta(dx, 0);
  }
  return ScrollAxisDelta(0, dy);
}
                ...
                ? Listener(
                    ...
                    onPointerSignal: (signal) {
                      if (signal is PointerScrollEvent) {
                        final delta = dominantAxisScrollDelta(
