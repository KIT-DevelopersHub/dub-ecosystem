// Right-rail palette. Click a tile to append that block to the canvas. Glyphs are
// plain characters/emoji so we stay off @dub/ui's closed IconName union.
...
type Entry = { type: BlockType; icon: string; label: string; desc: string };

const ENTRIES: Entry[] = [
  { type: "heading", icon: "H", label: "見出し", desc: "H1 / H2 / H3" },
  { type: "text", icon: "T", label: "テキスト", desc: "段落・説明" },
  { type: "checklist", icon: "✓", label: "チェックリスト", desc: "準備・TODO" },
...
export function BlockPalette({ onAdd, disabled }: Props) {
  return (
    <aside className={styles.palette} data-testid="fe3-blockeditor-palette">
