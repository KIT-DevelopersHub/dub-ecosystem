...
> **【必須・3 点セット】書き込み操作（追加・削除・並べ替え・親子/依存の付替え・日付編集など）は
> 必ず「①楽観的 UI ＋ ②成功トースト ＋ ③エラートースト＋ロールバック」をセットで実装する。**
> どれか 1 つでも欠けた編集 UI は不可（レビューで差し戻す）。「保存を押しても無反応」「時差があって
> 失敗したか不安」を二度と出さないための下限規約。

- **やり方**: TanStack Query の `onMutate`（または `qc.setQueryData`）で現在値を snapshot → キャッシュを
  **先に**新値へ更新して即描画 → 成功したら**成功トースト**（「保存しました」等）→ `onError` で snapshot に
  **ロールバック**＋**エラートースト**（握り潰さない）→ 最後にサーバ正を再取得（`invalidate`/refetch）。
- **共通ヘルパを使う**: トーストとエラー文言の生成は毎回書かず、共通フックに寄せる。fe4 は
  `useWriteFeedback()`（`src/domain/write-feedback.ts`）が `success(msg?)` / `failure(err, fallback?)` を返す
  （`failure` は `error-mapping` で人間可読な文言に変換して**エラートースト**を出す）。新しい編集 UI は
  この 2 つを呼ぶだけで③の下限を満たす。楽観キャッシュ更新の小関数は feature 側に（例:
  `useGanttData` の `setDependenciesOptimistic` / `setRowParentOptimistic`）。
- **対象**: トグル / 並び替え（D&D）/ 追加・削除 / 親子・依存（先行タスク）の付替え / インライン編集など、
  成功が高確率で結果が可逆な操作。**先行タスク（依存）の追加・削除も必ず楽観的に**（矢印を即時に増減させ、
  サーバ往復を待たせない）。
- **ローディングとの関係**: 楽観的更新できた操作では、その部分に**スピナー/スケルトンを出さない**
  （既に反映済みだから）。スケルトンは「まだ手元に無いデータの**初期取得**」に使い、楽観的 UI は
  「既にあるデータの**変更**」に使う——役割を混同しない。
- **例外**: 破壊的・不可逆（課金・確定送信など）は楽観的にせず、確認ダイアログ＋実ローディングにする。
  バー移動など**視覚的変化そのものがフィードバックになる連続操作**は成功トーストを省いてよい（連打で
  トーストが溢れるため）。ただし**失敗時のエラートースト＋ロールバックは省略不可**。
