# ADR-0007: Cross-scope task dependencies bounded by team
...
- Related: task-service (`services/task-service`), fe4-task-gantt (`apps/fe4-task-gantt`),
  ...
  cross-team request flow (design #377).
- Unifies: this is the single record for the whole feature. Earlier drafts split it —
  the frontend scope relaxation referenced it as "ADR-0006" and the backend门番 (PR #381)
  referenced it as "ADR-0007". Both refer to this one ADR. (ADR-0006 is the unrelated
  desktop-flutter-client decision.)
...
- Enforced in three places, all deriving the boundary from the same rule:
  1. **Backend门番** — `PUT /tasks/:id/dependencies` in `services/task-service/src/app.ts`
     ...
     Task-level team is already persisted — `task_tasks.team_id` was added additively by
     migration `task/0004_gantt_hierarchy.sql` (the gantt WBS/team-scope migration), so no
     new migration is required for this门番.
  2. **Frontend picker** — `apps/fe4-task-gantt/src/domain/task-hierarchy.ts`
     (`dependencyScopeOptions` / `pruneToScope` / `sameTeam`) offers same-team tasks across
     all scopes and excludes other teams, so the UI never proposes an edge the server would
     reject.
  3. **Mock client** — the fe4 `MockApiClient` mirrors the same-team gate so the demo /
     tests behave like production.
- The single boundary constant `DEPENDENCY_REJECT_REASONS.crossTeamNotAllowed` lives in
  `@dub/types` (`packages/types/src/task.ts`); client, server, and the server's regression
  test all import it so the literal `cross_team_not_allowed` can never drift
...
- **Arrow rendering (visual):** an arrow whose endpoint is a task hidden inside a collapsed
  parent is re-anchored to the **middle of that parent's bar**, keeping the edge legible
  while the child is folded. This is a **visual anchor only** — the critical-path (CPM)
  computation stays over the real child tasks and is never derived from the drawn
  coordinates.
...
- `task_tasks` already carries `team_id` (additive, nullable, from `0004_gantt_hierarchy`)
  and task create/update persist it; pre-existing rows keep `null` and stay valid.
