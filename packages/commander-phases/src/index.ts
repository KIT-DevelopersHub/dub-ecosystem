// Feature phase state machine — the SINGLE SOURCE OF TRUTH for the Dub development
// flow (`~/.claude/rules/dub-development-flow.md`), encoded as code so that two
...
//
// Pure module: no I/O, no DB. Both the local daemon and the commander-service worker
// import this module and call `transition()` on every move; illegal or unapproved
// moves throw. Persisting `Feature.phase` + the `PhaseTransition` audit log is the
// caller's job.
...
/** Every phase, in canonical lifecycle order (useful for UIs / validation). */
export const PHASES: readonly FeaturePhase[] = [
  "demo_building",
  "demo_review",
  "demo_rejected",
  "staging_deployed",
...
export function isFeaturePhase(v: unknown): v is FeaturePhase {
...
export type PhaseTransitionCode = "illegal_transition" | "approval_required";

export class PhaseTransitionError extends Error {
  readonly code: PhaseTransitionCode;
  /** Suggested HTTP status: 409 for 段飛ばし, 403 for 自己承認. */
  readonly httpStatus: 409 | 403;
  constructor(message: string, code: PhaseTransitionCode) {
    super(message);
    this.name = "PhaseTransitionError";
    this.code = code;
    this.httpStatus = code === "illegal_transition" ? 409 : 403;
  }
}
