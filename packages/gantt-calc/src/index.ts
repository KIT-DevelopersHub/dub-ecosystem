...
export type { Schedule, ScheduleNode } from "./schedule";
 ...
 */
export function computeRollup(req: ganttCalc.RollupRequest): ganttCalc.RollupResponse {
  const schedule = computeSchedule(req.tasks, req.dependencies);
  ...
  for (const t of req.tasks) {
    ...
    earliestStart[t.id] = isoAtDay(node.earliestStartDay);
    latestFinish[t.id] = isoAtDay(node.latestFinishDay);
  }
 ...
 */
export function computeCriticalPath(req: ganttCalc.CriticalPathRequest): ganttCalc.CriticalPathResponse {
  const schedule = computeSchedule(req.tasks, req.dependencies);
