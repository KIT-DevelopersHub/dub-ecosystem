// Real in-memory SQLite adapter exposing the subset of D1Database that @dub/freeq
// uses, seeded with the outbox DDL — so the outbox SQL (INSERT OR IGNORE, the
// status/next_attempt_at drain query, the UPDATEs) is exercised authentically, not
// mocked. Mirrors services/audit-log/test/d1.ts.
import { createRequire } from "node:module";
import type { D1Database } from "@cloudflare/workers-types";
import { OUTBOX_DDL } from "../src/outbox";
