...
const validClaims = () => ({ iss: "https://accounts.google.com", aud: AUD, exp: nowSec + 600, iat: nowSec });
...
describe("gmail OIDC verifier", () => {
  ...
  it("accepts a valid Google-signed push and derives the message id as dedup key", async () => {
    ...
    const res = await verify({ rawBytes: pubsubBody("msg-42"), headers: bearer(jwt) }, { gmailAudience: AUD });
  ...
  it("rejects a missing bearer token", async () => {
    const verify = createGmailVerifier({ fetch: jwksFetch(jwk), now });
    const res = await verify({ rawBytes: pubsubBody(), headers: new Headers() }, { gmailAudience: AUD });
    expect(res.ok).toBe(false);
  });
  ...
  it("rejects when the pub/sub message id is absent", async () => {
    ...
    const res = await verify({ rawBytes: body, headers: bearer(jwt) }, { gmailAudience: AUD });
    expect(res.ok).toBe(false);
  });
});
