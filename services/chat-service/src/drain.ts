...
// - evt.*        -> DEFERRED. The domain-event consumer (notification) has no free-tier
//   HTTP landing route yet, so the drain throws a benign deferral: the row stays
//   durable/pending and is redelivered later (once that route + SVC binding lands) —
//   never dropped, never mislabeled 'done'. Deferring also keeps the chat.* fan-out
//   asynchronous, so the drain never makes a synchronous call back into a consumer.
