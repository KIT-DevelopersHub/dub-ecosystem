...
describe("channel create", () => {
  it("creates a channel; creator becomes an admin member; emits channel.created + audit", async () => {
    ...
    const detail = await call(app, "GET", `/chat/channels/${res.json.id}`);
  ...
  it("event-type channel requires an eventId and existence check", async () => {
    ...
    const missing = await call(app, "POST", "/chat/channels", { body: { type: "event", visibility: "public", name: "E" } });
    expect(missing.status).toBe(400);

    const app2 = createApp(makeDeps({ eventClient: { async eventExists() { return false; } } }));
    const notFound = await call(app2, "POST", "/chat/channels", { body: { type: "event", visibility: "public", name: "E", eventId: "event_x" } });
  ...
  it("initial memberIds are added and emit member.added + realtime", async () => {
    const deps = makeDeps();
    const app = createApp(deps);
    const res = await call(app, "POST", "/chat/channels", { body: { ...topic, visibility: "private", memberIds: ["user_b"] } });
    expect(res.status).toBe(201);
    // user_b can now view the private channel
    const seen = await call(app, "GET", `/chat/channels/${res.json.id}`, { userId: "user_b" });
...
describe("channel visibility / existence hiding", () => {
  it("private channel: non-member GET -> 404 (hidden)", async () => {
    ...
    const other = await call(app, "GET", `/chat/channels/${c.json.id}`, { userId: "user_stranger" });
...
describe("channel list pagination (keyset by id)", () => {
  it("walks visible channels with no gaps or duplicates", async () => {
    ...
    const ids: string[] = [];
    for (let i = 0; i < 5; i++) {
      const c = await call(app, "POST", "/chat/channels", { body: { ...topic, name: `c${i}` } });
      ids.push(c.json.id);
    }
    const seen: string[] = [];
    ...
    for (let guard = 0; guard < 10; guard++) {
      ...
      const page = await call(app, "GET", "/chat/channels", { query: q });
  ...
  it("?eventId= filters to that event's channels", async () => {
    const app = createApp(makeDeps());
    await call(app, "POST", "/chat/channels", { body: { type: "event", visibility: "public", name: "E1", eventId: "event_1" } });
    await call(app, "POST", "/chat/channels", { body: topic });
    const page = await call(app, "GET", "/chat/channels", { query: { eventId: "event_1" } });
...
describe("channel update / archive (optimistic lock)", () => {
  it("patch name with correct version bumps version; stale version -> 409 CHAT_VERSION_CONFLICT", async () => {
    const app = createApp(makeDeps());
    const c = await call(app, "POST", "/chat/channels", { body: topic });
    const ok = await call(app, "PATCH", `/chat/channels/${c.json.id}`, { body: { version: 1, name: "Renamed" } });
    expect(ok.status).toBe(200);
    expect(ok.json.version).toBe(2);
    expect(ok.json.name).toBe("Renamed");

    const stale = await call(app, "PATCH", `/chat/channels/${c.json.id}`, { body: { version: 1, name: "X" } });
  ...
  it("archiving is audited and reflected in the channel", async () => {
    ...
    const app = createApp(deps);
    const c = await call(app, "POST", "/chat/channels", { body: topic });
    const arch = await call(app, "PATCH", `/chat/channels/${c.json.id}`, { body: { version: 1, archived: true } });
...
describe("membership add / remove events", () => {
  it("add member emits chat.member.added; self-remove emits chat.member.removed", async () => {
    ...
    const add = await call(app, "POST", `/chat/channels/${c.json.id}/members`, { body: { userId: "user_b" } });
    expect(add.status).toBe(204);
    expect(deps.publisher.namesFor("chat.member.added")).toHaveLength(1);

    // user_b removes self
    const del = await call(app, "DELETE", `/chat/channels/${c.json.id}/members/user_b`, { userId: "user_b" });
