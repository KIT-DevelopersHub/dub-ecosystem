...
describe("authentication + channel-scoped authorization", () => {
  it("unauthenticated request (no x-dub-user-id) -> 401", async () => {
    ...
    const res = await call(app, "GET", "/channels", { userId: null });
  ...
  it("non-member cannot POST to a public channel -> 403", async () => {
    ...
    const res = await call(app, "POST", "/messages", { userId: "user_stranger", body: { channelId: c.json.id, body: "hi" } });
  ...
  it("non-member cannot list a public channel's messages -> 403", async () => {
    const app = createApp(makeDeps());
    const c = await call(app, "POST", "/channels", { body: topic });
    const res = await call(app, "GET", "/messages", { userId: "user_stranger", query: { channelId: c.json.id } });
    expect(res.status).toBe(403);
  });
