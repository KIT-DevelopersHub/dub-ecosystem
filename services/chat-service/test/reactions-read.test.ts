...
describe("reactions (toggle, idempotent)", () => {
  it("toggles a reaction on and off for the caller", async () => {
    ...
    const m = await call(app, "POST", "/messages", { body: { channelId: c.json.id, body: "react to me" } });

    const on = await call(app, "POST", `/messages/${m.json.id}/reactions`, { body: { emoji: "👍" } });
    expect(on.status).toBe(200);
    expect(on.json.reactions["👍"]).toEqual(["user_caller"]);

    const off = await call(app, "POST", `/messages/${m.json.id}/reactions`, { body: { emoji: "👍" } });
  ...
  it("non-member cannot react -> 403", async () => {
    ...
    const m = await call(app, "POST", "/messages", { body: { channelId: c.json.id, body: "x" } });
    const res = await call(app, "POST", `/messages/${m.json.id}/reactions`, { userId: "user_stranger", body: { emoji: "🎉" } });
...
describe("read state + unread", () => {
  it("unread counts others' messages, excludes own, and drops after marking read", async () => {
    ...
    await call(app, "POST", "/messages", { body: { channelId, body: "mine" } });
    await call(app, "POST", "/messages", { userId: "user_b", body: { channelId, body: "b1" } });
    const last = await call(app, "POST", "/messages", { userId: "user_b", body: { channelId, body: "b2" } });

    const unread1 = await call(app, "GET", "/unread");
    ...
    const read = await call(app, "POST", `/channels/${channelId}/read`, { body: { lastReadMessageId: last.json.id } });
    expect(read.status).toBe(200);
    expect(read.json.unreadCount).toBe(0);

    const unread2 = await call(app, "GET", "/unread");
  ...
  it("unread only includes channels the caller is a member of", async () => {
    ...
    const other = await call(app, "POST", "/channels", { userId: "user_b", body: { ...topic, visibility: "private", name: "b-only" } });
    await call(app, "POST", "/messages", { userId: "user_b", body: { channelId: other.json.id, body: "hidden" } });

    const unread = await call(app, "GET", "/unread");
