...
function toParticipationRow(r: ParticipationDbRow): ParticipationRow {
  return {
    ...
    undoSnapshot: r.undo_snapshot ?? null,
      ...
      await db.run(
        `INSERT INTO member_participations
          (id, org_id, member_id, name, normalized_name, last_name, first_name, name_kana,
           ...
           school_email, gmail, desired_team_id, desired_activity, note, status, match_kind, review_state, undo_snapshot,
        ...
        row.note, row.status, row.matchKind, row.reviewState, row.undoSnapshot, row.submittedBy, row.submittedAt, row.createdAt, row.updatedAt,
      );
    },
