...
describe("HTTP app", () => {
  ...
  it("GET /inbox?sort=oldest returns rows created-ascending (default is newest-first)", async () => {
    const h = makeTestEnv();
    await req("/notify", internalPost({ type: "system.announcement", recipientIds: ["u1"], title: "first", body: "b", channels: ["in_app"] }), h);
    await req("/notify", internalPost({ type: "system.announcement", recipientIds: ["u1"], title: "second", body: "b", channels: ["in_app"] }), h);
    await req("/notify", internalPost({ type: "system.announcement", recipientIds: ["u1"], title: "third", body: "b", channels: ["in_app"] }), h);

    const newest = (await (await req("/inbox", { headers: { "x-dub-user-id": "u1" } }, h)).json()) as { items: { title: string }[] };
    const oldest = (await (await req("/inbox?sort=oldest", { headers: { "x-dub-user-id": "u1" } }, h)).json()) as { items: { title: string }[] };
    // sort=oldest is the exact reverse of the default (newest-first) ordering.
    expect(newest.items).toHaveLength(3);
    expect(oldest.items.map((i) => i.title)).toEqual([...newest.items.map((i) => i.title)].reverse());
  });
