...
describe("POST /auth/password/login", () => {
  ...
  it("valid credentials -> 200, Set-Cookie web session, verifiable token", async () => {
    const h = makeHarness();
    await seed(h, "admin@dub.local", "demo-admin-pw");
    // identity resolves the canonical active user (FakeIdentity default = existing)
    const app = buildApp(h.deps);
    const res = await app.request("/auth/password/login", jsonInit({ email: "admin@dub.local", password: "demo-admin-pw" }));
  ...
  it("is case-insensitive on the email", async () => {
    const h = makeHarness();
    await seed(h, "admin@dub.local", "pw");
    const app = buildApp(h.deps);
    const res = await app.request("/auth/password/login", jsonInit({ email: "ADMIN@DUB.LOCAL", password: "pw" }));
    expect(res.status).toBe(200);
  });

  it("wrong password -> 401 AUTH_INVALID_CREDENTIALS", async () => {
    const h = makeHarness();
    await seed(h, "admin@dub.local", "right");
    const app = buildApp(h.deps);
    const res = await app.request("/auth/password/login", jsonInit({ email: "admin@dub.local", password: "wrong" }));
  ...
  it("unknown email -> 401 with the SAME code (no account enumeration)", async () => {
    const h = makeHarness();
    const app = buildApp(h.deps);
    const res = await app.request("/auth/password/login", jsonInit({ email: "nobody@dub.local", password: "whatever" }));
    expect(res.status).toBe(401);
    expect(((await res.json()) as { error: { code: string } }).error.code).toBe("AUTH_INVALID_CREDENTIALS");
  });
  ...
  it("invite-only: valid password but rejected identity -> 403 AUTH_USER_REJECTED", async () => {
    const h = makeHarness();
    await seed(h, "ghost@dub.local", "pw");
    h.identity.result = { status: "rejected", user: null };
    const app = buildApp(h.deps);
    const res = await app.request("/auth/password/login", jsonInit({ email: "ghost@dub.local", password: "pw" }));
  ...
  it("rate limits repeated failures for the same email -> 429 RATE_LIMITED", async () => {
    ...
    await seed(h, "admin@dub.local", "right");
    const app = buildApp(h.deps);
    const attempt = () => app.request("/auth/password/login", jsonInit({ email: "admin@dub.local", password: "wrong" }));
    ...
    const blocked = await app.request("/auth/password/login", jsonInit({ email: "admin@dub.local", password: "right" }));
