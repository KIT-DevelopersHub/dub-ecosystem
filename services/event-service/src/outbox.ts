// Free-tier outbox shim (@dub/freeq). event-service's event.*/action.* fan-out and
// its audit channel used paid Cloudflare Queue producers (EVT_* / AUDIT_QUEUE).
...
// and mail-gateway freeq conversions.
...
// Audit topic — matches the auth-service / mail-gateway convention (audit-log's async
// ingest understands the AuditRecordEnvelopeV1 payload verbatim).
export const AUDIT_TOPIC = "audit.record";

// Outbox topic for a domain-event consumer (evt.notification, evt.task, ...). Stable
// strings so the drain can route a row to the right consumer and operators can query
// freeq_outbox by topic. Derived from the frozen consumer key (already hyphenated,
// e.g. "file-meta" -> "evt.file-meta").
export function eventTopic(consumer: ConsumerService): string {
  return `evt.${consumer}`;
}

// Every domain-event topic event-service can fan out to. The drain DEFERS these (no
// free-tier consumer HTTP route yet) rather than dropping them: keeping the fan-out
// asynchronous also avoids re-introducing the event<->task synchronous cycle.
