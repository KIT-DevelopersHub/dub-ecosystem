...
// retry/backoff bookkeeping; delivery is at-least-once and the envelope id is the
...
// - evt.*         -> DEFERRED. The domain-event consumers (notification / github-sync /
//   gantt / mobile-bff / file-meta) are reached over their own free-tier HTTP landing
//   routes, not from task-service; until those SVC bindings are wired here the drain throws
//   a benign deferral so the row stays durable/pending (never dropped, never mislabeled
//   'done'). A high maxAttempts keeps deferred rows off the terminal 'failed' state.
