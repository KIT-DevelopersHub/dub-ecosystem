...
export function buildApp(deps: Deps): Hono {
  ...
  app.put("/tasks/:id/dependencies", async (c) => {
    ...
    // Team gate (Feature 1 / ADR-0007): a dependency (arrow) may only join tasks of the
    // SAME team. `team_id === null` counts as its own "no team" bucket, so two teamless
    // tasks may still depend (D2, back-compat); a one-sided null is a mismatch. Cross-team
    // links go through the request/approval flow instead — never a dependency arrow. We
    // only flag candidates that exist in this bucket; unknown ids fall through to the
