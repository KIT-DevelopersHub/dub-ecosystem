-- task-service free-tier outbox (@dub/freeq). Free-plan replacement for the paid
...
-- it is infra plumbing, not @dub/db task_ business-namespace data, so it never collides
-- with the task_* tables (task_tasks / task_dependencies / task_idempotency).
-- Keep in lockstep with @dub/freeq OUTBOX_DDL. Applied out-of-band (like mail-gateway
-- 0003_freeq_outbox.sql), NOT part of the task-namespace TASK_MIGRATIONS.
