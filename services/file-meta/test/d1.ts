// Real in-memory SQLite adapter exposing the subset of D1Database that @dub/freeq uses,
// seeded with file-meta's freeq_outbox migration — so the outbox SQL (INSERT OR IGNORE,
// the status/next_attempt_at drain query, the UPDATEs) is exercised authentically, not
// mocked. Mirrors services/mail-gateway/test/d1.ts.
...
// Load node:sqlite through createRequire so Vite/Vitest's transform pipeline never tries
// to resolve it as a bundleable module (it strips the "node:" prefix and 404s).
