...
import { AUDIT_TOPIC, TOPIC_MAIL_AUTOMATION, TOPIC_NOTIFICATION } from "../src/outbox";
...
const auditEnvelope = {
  ...
  payload: {
    ...
    resourceId: "msg_1@developershub.jp",
    details: { requester: "usr_alice" },
    requestId: "req_1",
...
describe("runOutboxDrain", () => {
  ...
  it("defers domain-event rows (stay pending, not delivered)", async () => {
    const { d1, raw } = makeD1();
    await enqueue(d1 as D1Database, TOPIC_NOTIFICATION, { name: "mail.message.sent", id: "e1" });
    await enqueue(d1 as D1Database, TOPIC_MAIL_AUTOMATION, { name: "mail.message.received", id: "e2" });
    const { svc, calls } = recordingAudit(202);
    ...
    expect(statusOf(raw, TOPIC_NOTIFICATION)).toBe("pending");
    expect(statusOf(raw, TOPIC_MAIL_AUTOMATION)).toBe("pending");
  });
