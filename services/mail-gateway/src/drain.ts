...
// - evt.*         -> DEFERRED. The domain-event consumers (mail-automation / notification)
//   have no free-tier HTTP landing route yet, so the drain throws a benign deferral: the
//   row stays durable/pending and is redelivered later (once those routes + SVC bindings
//   land) — it is never dropped and never mislabeled 'done'. A high maxAttempts keeps
//   deferred rows off the terminal 'failed' state.
