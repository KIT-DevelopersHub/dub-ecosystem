...
// - evt.*         -> DEFERRED. The domain-event consumer (notification) is reached over its
//   own free-tier HTTP landing route, not from github-sync; until that SVC binding is wired
//   here the drain throws a benign deferral so the row stays durable/pending (never dropped,
//   never mislabeled 'done'). A high maxAttempts keeps deferred rows off the terminal
//   'failed' state.
