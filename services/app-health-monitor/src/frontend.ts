...
//   1. ONE asset sweep: GET the build-emitted /app-health.json (list of every JS/CSS chunk the
//      SPA ships) and probe each asset exists (200). A single 404 here breaks the whole SPA.
...
import { feOrigin, FRONTEND_APPS, APP_HEALTH_MANIFEST_PATH } from "./config";
...
/** GET /app-health.json and probe every listed asset. A missing manifest is itself a failure
 *  (the build/deploy didn't ship it) but is reported distinctly from a missing chunk. */
export async function sweepAssets(origin: string): Promise<AssetSweep> {
  const manifestUrl = `${origin}${APP_HEALTH_MANIFEST_PATH}`;
  let manifest: AppHealthManifest;
  try {
    const res = await fetch(manifestUrl, { redirect: "manual" });
  ...
  for (const asset of assets) {
    const url = asset.startsWith("http") ? asset : `${origin}${asset.startsWith("/") ? "" : "/"}${asset}`;
    // HEAD first (cheap); some asset servers 405 HEAD, so fall back to GET on non-200/404.
    let out = await probeHttp(url, { method: "HEAD" });
    if (!out.ok && !/HTTP 404/.test(out.detail)) out = await probeHttp(url, { method: "GET" });
    if (!out.ok) missing.push(`${asset} (${out.detail.split(" ")[1] ?? "fail"})`);
  }
  if (missing.length > 0) {
