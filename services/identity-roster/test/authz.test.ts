...
async function seedUser(repo: MemIdentityRepo, id: string, status: identity.UserStatus = "active", orgId = ORG) {
  await repo.createUser({ id, orgId, email: `${id}@x.jp`, displayName: id, furigana: null, githubLogin: null, avatarUrl: null, status, source: "manual", createdAt: TS, updatedAt: TS });
}
